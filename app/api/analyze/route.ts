...
function detectFormat(buf: Buffer): "jpeg" | "png" | "bmp" | "raw" {
  ...
  return "raw" // grayscale puro do OV7670
}

// ── Converte qualquer coisa para JPEG em memória ──────────────────────────────
...
export async function POST(request: Request) {
  try {
    ...
    let actuators: { name: string; type: string; status: boolean; consumption: number }[] = []
    let image_url: string | undefined

    // ── ESP32: bytes binários ─────────────────────────────────────
    if (
    ...
    // ── Dashboard: JSON normal ────────────────────────────────────
      ...
      if (body.image_base64) {
        ...
        if (raw64.startsWith("data:")) {
          ...
          if (match) { mime = match[1]; raw64 = match[2] }
        }

        const buf = Buffer.from(raw64, "base64")
        // Converte se necessário (png/bmp → jpeg)
...
Responda EXCLUSIVAMENTE em JSON válido (sem markdown, sem \`\`\`) no formato:
{
  ...
  "next_action": "<ação mais urgente agora>"
    ...
    let parsed: {
      name?: string; species?: string; health_score?: number; status?: string
      problems?: string[]; recommendations?: string[]; summary?: string
      growth_stage?: string; next_action?: string
    } = {}

    try { parsed = JSON.parse(cleaned) }
    catch { parsed = { summary: cleaned, health_score: 70, problems: [], recommendations: [] } }

    // ── Busca planta pela posição ─────────────────────────────────
    const { data: nearby } = await supabase
      .from("plants").select("id, position_x, position_y")
    ...
    if (nearby) {
      ...
      await supabase.from("plants").update({
        name: parsed.name ?? "Planta", species: parsed.species ?? null,
        health_score: parsed.health_score ?? 100,
        status: parsed.status ?? "healthy", image_url: savedImageUrl,
      }).eq("id", plantId)
    } else {
    ...
    if (sensors.length > 0) {
      await supabase.from("sensors").insert(
        sensors.map((s) => ({ plant_id: plantId, type: s.type, value: s.value, unit: s.unit ?? null }))
      )
    }

    // ── Atuadores ─────────────────────────────────────────────────
    for (const a of actuators) {
      const { data: existing } = await supabase.from("actuators").select("id")
        .eq("plant_id", plantId).eq("name", a.name).single()

      if (existing) {
        await supabase.from("actuators").update({ type: a.type, status: a.status, consumption: a.consumption })
          .eq("id", existing.id)
      } else {
        await supabase.from("actuators").insert({
          plant_id: plantId, name: a.name, type: a.type, status: a.status, consumption: a.consumption,
        })
      }
    }
    ...
    const { data: report, error: reportError } = await supabase
      .from("ai_reports").insert({
        plant_id: plantId, image_url: savedImageUrl,
        health_score: parsed.health_score ?? null,
        problems: parsed.problems ?? [], recommendations: parsed.recommendations ?? [],
        summary: parsed.summary ?? null, raw_response: parsed,
      }).select().single()

    if (reportError) return NextResponse.json({ error: reportError.message }, { status: 500 })

    return NextResponse.json({ plant_id: plantId, is_new_plant: isNew, image_url: savedImageUrl, report, analysis: parsed })
